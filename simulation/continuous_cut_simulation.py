...
_C = dict(
    outer       = "#1E6FBF",
    hole        = "#D9534F",   # Innenrand (Lochkontur) -- rot/orange
    inner       = "#AAAAAA",
    cut_point   = "#22A84E",
    grip        = "#FFD700",
    grip_edge   = "#B8860B",
...
class ContinuousCutSimulation:
    ...
    def _draw_main(self) -> None:
        ...
        # Innenrandpunkte (Lochkontur) -- Y-Fase-faehig
        hole = self.grid.hole_points
        if hole:
            ax.scatter([p.x for p in hole], [p.y for p in hole],
                       s=48, color=_C["hole"], edgecolors="#7A1F1A",
                       linewidths=0.7, zorder=3, marker="D")
        ...
        ax.legend(
            handles=[
                ...
                Line2D([0], [0], marker="D", color="w",
                       markerfacecolor=_C["hole"], markeredgecolor="#7A1F1A",
                       markersize=8, label="Innenrand (Loch)"),
...
# ---------------------------------------------------------------------------
# Startauswahl-Dialog (Matplotlib)
# ---------------------------------------------------------------------------

def _select_startup_options(
    default_preset: str = "realistic",
    default_thickness: float = 12.0,
) -> tuple[str, float] | None:
    """Matplotlib-Startfenster fuer Preset-Auswahl und Blechstaerke.

    Returns
    -------
    (preset, thickness) wenn der User auf Start klickt, sonst None
    (Fenster geschlossen ohne Bestaetigung).
    """
    presets = ["ideal", "realistic", "noisy"]
    descriptions = {
        "ideal":     "Referenz: alle Annahmen aus.\n"
                     "L = konstant, kein Pierce, keine Y-Fase, kein Tilt-Noise.",
        "realistic": "Default: L(v) inverse, Pierce-Zeit nach Blechdicke,\n"
                     "45°-Y-Fase an Lochkontur. Tilt-Noise aus.",
        "noisy":     "Wie realistic + Brennerwinkel-Streuung (OU-Prozess,\n"
                     "Sigma=2°, max_tilt=5°). Realitaetsnaechstes Modell.",
    }
    ...
    try:
        fig.canvas.manager.set_window_title("Plasmaschneider – Startauswahl")
    except Exception:
        pass
    fig.suptitle("Plasmaschneider-Simulation",
                 fontsize=14, fontweight="bold", y=0.95, color="#1A2840")
    fig.text(0.5, 0.87, "Annahmen-Preset auswaehlen",
             fontsize=10.5, ha="center", color="#445566")

    # Radiobuttons
    ax_radio = fig.add_axes([0.06, 0.20, 0.36, 0.60], frameon=True)
    ax_radio.set_facecolor("#F4F7FB")
    radio = RadioButtons(
        ax_radio, presets,
        active=presets.index(default_preset),
        activecolor="#2E6FBF",
    )
    ...
    # Beschreibungspanel
    ax_desc = fig.add_axes([0.46, 0.40, 0.48, 0.40], frameon=True)
    ax_desc.set_facecolor("#FFFDF5")
    ax_desc.set_xticks([])
    ax_desc.set_yticks([])
    desc_txt = ax_desc.text(
        0.04, 0.5, descriptions[default_preset],
        fontsize=9.5, color="#333333", va="center", ha="left",
        wrap=True, transform=ax_desc.transAxes,
    )

    # Blechstaerke
    ax_thick = fig.add_axes([0.60, 0.27, 0.16, 0.08])
    tb_thick = TextBox(ax_thick, "Blechstaerke [mm]  ",
                       initial=str(default_thickness))

    # Start-Button
    ax_btn = fig.add_axes([0.78, 0.20, 0.16, 0.10])
    btn = Button(ax_btn, "Start", color="#2E8B57", hovercolor="#3CB371")
    ...
    def _on_preset(label: str) -> None:
        state["preset"] = label
        desc_txt.set_text(descriptions[label])
        fig.canvas.draw_idle()

    radio.on_clicked(_on_preset)

    def _on_start(_event) -> None:
        try:
            state["thickness"] = float(tb_thick.text.replace(",", "."))
        except ValueError:
            state["thickness"] = default_thickness
        state["ok"] = True
        plt.close(fig)

    btn.on_clicked(_on_start)

    plt.show()

    if not state["ok"]:
        return None
    return state["preset"], float(state["thickness"])
...
if __name__ == "__main__":
    ...
    try:
        from ..cutter.assumptions import (
            preset_ideal, preset_realistic, preset_noisy,
        )
    except ImportError:
    ...
    parser.add_argument(
        "--preset", choices=["ideal", "realistic", "noisy"],
        default="realistic",
        help="Annahmen-Preset: ideal | realistic (default) | noisy",
    )
    parser.add_argument(
        "--thickness", type=float, default=12.0,
        help="Blechstaerke in mm (beeinflusst Pierce-Zeit)",
    )
    parser.add_argument(
        "--no-dialog", action="store_true",
        help="Startauswahl-Dialog ueberspringen (CLI-Werte direkt verwenden)",
    )
    args = parser.parse_args()

    preset_factories = {
        "ideal":     preset_ideal,
        "realistic": preset_realistic,
        "noisy":     preset_noisy,
    }
    ...
    if args.no_dialog:
        ...
        selection = _select_startup_options(
            default_preset=args.preset,
            default_thickness=args.thickness,
        )
        if selection is None:
            print("Abbruch durch User (Startauswahl geschlossen).")
            import sys
            sys.exit(0)
        preset_name, thickness = selection

    assumptions = preset_factories[preset_name]()
    assumptions.sheet_thickness = thickness

    kw: dict = {"assumptions": assumptions}
    ...
    cutter = Cutter(**kw)
    print(f"Preset: {preset_name}  |  Blechstaerke: {thickness:.1f} mm")
