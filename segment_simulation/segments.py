...
class ContourLoop:
    ...
    kind    : "outer" (Außenrand) oder "hole" (Lochrand)
    ...
    def arc_length(self, a: int, b: int, direction: int) -> float:
        """Bogenlänge von a nach b in der gegebenen Richtung [mm]."""
    ...
    def polyline(self, positions: list[int]) -> np.ndarray:
        """(M, 2) Koordinaten-Array für eine Positionsfolge."""
...
class CutRun:
    ...
    length        : Konturlänge des Schnitts [mm]
    ...
    def reversed(self) -> CutRun:
        """Derselbe Schnitt in Gegenrichtung (für den Sequencer)."""
        return CutRun(
            run_id=self.run_id,
...
class SegmentedContour:
    ...
    # ------------------------------------------------------------------
    # Material-Polygon (für Kollisionsprüfung)
    # ------------------------------------------------------------------

    def material_polygon(self) -> Polygon | MultiPolygon | None:
        """Material = Außenkontur abzüglich aller Lochkonturen."""
        outer_poly = None
        holes: list[Polygon] = []
    ...
    def _segment_loop(self, loop: ContourLoop) -> None:
        ...
        # Bögen zwischen aufeinanderfolgenden Ecken unterteilen
        ...
        for ci in range(n_corners):
            ...
            if n_parts <= 1:
                continue
            # Knoten an gleichmäßigen Bogenlängen-Abständen einfügen
        ...
        for nd in nodes:
            ...
            if nd - filtered[-1] < 2:
                ...
                continue
            filtered.append(nd)
        # Zyklischen Abstand zwischen letztem und erstem Knoten prüfen
    ...
    def snap_node(
        ...
        """Findet den nächstgelegenen Segment-Knoten zu einem Klick.
    ...
    def node_positions_of(self, loop_id: int,
        ...
        """Die Segmentknoten unter ``positions`` (Stützstellen des
        TCP-Pfads; Endpunkte sind immer dabei)."""
...
class CoverageReport:
    ...
    def fraction(self) -> float:
        """Abdeckung [0..1] über alle Konturen."""
        t = self.total_points
...
def compute_coverage(
    ...
    """Berechnet die Konturabdeckung der gewählten CutRuns."""
...
def covered_positions(
    ...
    """Abgedeckte Positionen eines Loops (Hilfsfunktion für UI)."""
    covered: set[int] = set()
    for run in runs:
...
# ---------------------------------------------------------------------------
# Querschnitts-Coverage (alle Gitterpunkte: innen + außen)
# ---------------------------------------------------------------------------
