...
def test_baseline_replay(cfg: RLConfig) -> dict:
    ...
    for f in train_files:
        try:
            ...
            # Der Phase-1-`calculate_reward` nutzt quadratisches Coverage-
            # Shaping, waehrend der Env-Reward seit Phase 5b linear ist.
            # Ein 1:1-Wertvergleich ist damit nicht mehr moeglich. Wir
            # pruefen stattdessen, dass das Replay der Baseline-Waypoints
            # mechanisch sauber durchlaeuft: kein infeasible, hohe Coverage,
            # und der Return liegt in der Groessenordnung, die die lineare
            # Formel fuer diese Trajektorie vorhersagt.
            final_cov  = info["coverage"]
            final_time = info["total_time"]
            is_feasible = info.get("is_feasible", True)

            expected_linear = (
                cfg_test.reward.w_coverage_delta * final_cov
                - cfg_test.reward.w_time_delta   * final_time
                - cfg_test.reward.w_pierce * max(0, info["n_pierces"] - 1)
            )
            if final_cov >= cfg_test.reward.completion_threshold:
                expected_linear += cfg_test.reward.w_completion

            diff   = abs(total_env_reward - expected_linear)
            passed = is_feasible and (final_cov >= 0.5) and (diff < 5.0)
            all_passed = all_passed and passed

            results[f.name] = {
                "env_reward":      float(total_env_reward),
                "expected_linear": float(expected_linear),
                "diff":            float(diff),
...
def test_reward_decomposition(cfg: RLConfig) -> dict:
    ...
    for seed in range(42, 52):
        ...
        #     w_cov * cov_final  -  w_time * total_time
        # (lineares Potential, siehe mdp.compute_step_reward). Gilt
        # unabhaengig davon, ob die Episode regulaer endet oder mit
        # einem Infeasible-Step abgebrochen wird -- die Env behaelt in
        # beiden Faellen den step_reward des letzten Steps.
        dense = (
            cfg.reward.w_coverage_delta * info["coverage"]
