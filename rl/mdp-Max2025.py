...
class ObservationContext:
    ...
    geometry_mask_u8: (H, W) uint8 -- 0/255-Version der gleichen Maske,
                      vorab gecastet, damit `render_observation` pro Step
                      keinen astype-Allokationsweg laufen muss.
    ...
    geometry_mask_u8: np.ndarray   # shape (H, W), dtype uint8 (0 or 255)
    bbox_diagonal:    float
...
def build_observation_context(
    ...
    # uint8-Version einmal vorab casten. `render_observation` laeuft pro
    # Step und wuerde sonst H*W Pixel pro Aufruf von bool nach uint8
    # kopieren -- bei 128*128*16-Worker*1024-Steps summiert sich das.
    geometry_mask_u8 = np.where(geometry_mask, np.uint8(255), np.uint8(0))
    ...
    return ObservationContext(
        ...
        geometry_mask_u8 = geometry_mask_u8,
...
def render_observation(
    ...
    """Rendert die Observation als (C, H, W)-uint8-Array.

    Die Observation ist uint8 [0, 255] statt float32 [0, 1], weil SB3-
    SubprocVecEnv unter Windows jeden Step den kompletten Obs-Block ueber
    eine Named-Pipe pickelt. Bei 16 Workern, 128*128*3 float32 = 196 kB
    pro Step und 16384 Steps/Rollout landen rund 50 GB Pickle-Traffic im
    Master-Prozess -- ein massiver Python-seitiger Bottleneck. Mit uint8
    sind es 49 kB/Obs und damit nur noch ~12.5 GB/Rollout -- Faktor 4
    weniger pickle- und pipe-Durchsatz, plus entsprechend weniger RAM im
    Rollout-Buffer.

    SB3 kuemmert sich anschliessend um das /255-Normalisieren auf der
    GPU: `normalize_images=True` (Default der CnnPolicy) dividiert den
    Tensor vor dem ersten Conv-Layer. Die rekonstruierten [0, 1]-Werte
    sind mit < 1/255 Quantisierungsfehler identisch zum bisherigen
    float32-Pfad, d.h. die Lernqualitaet bleibt 1:1 erhalten.
    ...
    Returns
    -------
    obs : (C, H, W) uint8 mit C = Summe der aktivierten Kanaele
    """
    H = W = ctx.resolution
    n_channels = (
    ...
    obs = np.empty((n_channels, H, W), dtype=np.uint8)
    idx = 0

    if cfg.observation.use_geometry_channel:
        # Vorab gecastete uint8-Version aus dem Kontext -- copy statt
        # astype, damit der Rollout-Buffer nicht auf shared memory
        # aliasiert (SB3 kopiert zwar ohnehin, aber wir wollen uns nicht
        # darauf verlassen).
        np.copyto(obs[idx], ctx.geometry_mask_u8)
        idx += 1
    ...
    if cfg.observation.use_coverage_channel:
        ...
        # bool -> uint8(0/255) direkt in den Output-Slice schreiben.
        np.multiply(cm, np.uint8(255), out=obs[idx], casting="unsafe")
        idx += 1

    if cfg.observation.use_tcp_channel:
        # Gauss-Blob liegt weiter in float32 [0, 1] vor -- wir quanti-
        # sieren ihn beim Schreiben in den uint8-Output-Slice. 0..1 mal
        # 255 = 0..255, ohne Overflow-Risiko.
        blob = _draw_tcp_blob(
            tcp_world=tcp_world,
            ctx=ctx,
            sigma_px=cfg.observation.tcp_blob_sigma_px,
        )
        np.multiply(blob, 255.0, out=blob)
        np.copyto(obs[idx], blob, casting="unsafe")
        idx += 1
...
# ===========================================================================
#
# Der Reward ist potential-basiert zerlegt, die Summe ueber eine Episode
# teleskopiert zu:
#
#     sum_t [ w_cov * (cov_t - cov_{t-1}) - w_time * Δt_t ] + terminal
#   = w_cov * cov_T  -  w_time * total_time  +  terminal
#
# Lineares (statt quadratisches) Potential: belohnt Fortschritt am Anfang
# gleich stark wie am Ende -- das hilft, das Plateau bei 0% Coverage zu
# ueberwinden. Dadurch weicht der Return absolut vom Phase-1-Baseline-
# Reward ab; die Vergleichbarkeit laeuft nur noch qualitativ (mehr Cov /
# weniger Zeit = mehr Reward), nicht mehr wertgleich.
#
# Sonderfaelle:
...
def episode_return_upper_bound(cfg: RLConfig) -> float:
    ...
    """
    return float(
        cfg.reward.w_coverage_delta * 1.0  # cov = 1 (lineares Potential)
