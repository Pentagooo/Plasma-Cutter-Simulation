...
def load_raw_contour(path: Path) -> tuple[list[list[float]], list[list[float]] | None]:
    ...
    if isinstance(data, dict):
        ...
        # Wie umgesetzt: Die "or"-Kette nimmt den ersten Schlüssel, der
        #   einen Wert liefert (alles andere ergibt None/leer = "falsy").
        outer_raw = (
            data.get("outer")
            or data.get("points")
...
def densify_contour(
    ...
    Wie umgesetzt:
        ...
        Punkt betrachten (zyklisch, damit der letzte mit dem ersten Punkt
        einen geschlossenen Ring bildet). Ist das Segment zu lang, werden
        per linearer Interpolation p0 + t*(p1-p0) gleichmäßig verteilte
        Zwischenpunkte (t = 1/n ... (n-1)/n) eingefügt. math.ceil sorgt
        dafür, dass die Teilstücke garantiert <= max_dist werden.
...
def generate_inner_points(
    ...
    Wie umgesetzt:
        Aus den Konturpunkten wird ein shapely-Polygon gebaut (Loch als
        zweiter Ring). polygon.bounds liefert das umschließende Rechteck
        (Bounding Box). Darüber läuft eine doppelte while-Schleife in
        x und y mit Schrittweite "spacing". Für jeden Rasterpunkt fragt
        polygon.contains(Point(x, y)), ob er wirklich im Werkstück liegt
        (Löcher zählen automatisch als "außen"). Nur solche Treffer
        landen in der Ergebnisliste.
...
def prepare_geometry_points(
    ...
    """Vollständige Verarbeitung: Laden -> Verdichten -> Innenpunkte -> Speichern.
    ...
    # Alle Punktarten in EINE flache Liste schreiben, jeweils mit "type"-
    # Markierung. Reihenfolge: erst Außenkontur, dann Loch, dann Raster.
    # So kann ein Leser später am "type"-Feld erkennen, welche Rolle ein
    # Punkt hat, ohne die Listen getrennt halten zu müssen.
    points_list = []
...
def _draw_contour(
    ...
    # Achsgrenzen automatisch aus allen sichtbaren Punkten ableiten.
    # Wie umgesetzt: größte Ausdehnung (span) bestimmen und daraus einen
    # Rand "m" (10 % plus 5 mm Sockel) berechnen, damit die Geometrie nicht
    # am Bildrand klebt. set_aspect("equal") hält x/y maßstabsgetreu.
...
def main() -> None:
    """Einstiegspunkt für den Aufruf von der Kommandozeile.
    """
    ...
    # Wie umgesetzt: tkinter wird erst HIER importiert (nicht oben), damit
    #   das Skript auch ohne GUI läuft, solange ein Pfad übergeben wird.
    #   root.withdraw() versteckt das leere Hauptfenster; "-topmost" holt
    #   den Dialog in den Vordergrund.
